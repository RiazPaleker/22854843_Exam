over_perform_summer <- function(GDP,summer){
    ...
    # Plot using ggplot2
    gdp_medal_summer_plot <- ggplot(medal_count, aes(x = LogGDP, y = TotalMedals)) +
        geom_point(alpha=0.8, size =2) + geom_text(aes(label = Country), vjust = -1, hjust = 0.5, size=2.9)+
        scale_color_viridis_d()+
        labs(title = "Medal Count vs GDP per Capita (Summer Olympics)",
             x = "Log GDP per Capita",
             y = "Total Medal Count")+
        ylim(5, 750)+ xlim(6,12)+
        geom_vline(xintercept = quartiles[1], linetype = "dashed", color = "red",) +
        geom_vline(xintercept = quartiles[2], linetype = "dashed", color = "red") +
        geom_vline(xintercept = quartiles[3], linetype = "dashed", color = "red")+
